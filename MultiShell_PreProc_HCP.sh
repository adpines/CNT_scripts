...
rm -r /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/AMICO/NODDI
...
#	Clean up old files
	...
	rm /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/AMICO/*
# Make directory structure
	...
	# Merge all Shells into one nifti because that's what I'm used to and change is scary
		...
		# all shells
	####cp $general/HCP_Multishell*/* /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells
	####cd /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells
	####dcm2nii *

	#cp $general/NODDI_B_2000_Series0013/* /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells
	#cd /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells
	#series=/data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells/*
	#for i in $series; do 
	#	dicom=$(echo ${i}|cut -d'/' -f10 |sed s@'/'@' '@g)
	...
	#cp $general/NODDI_B_300_Series0015/* /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells
	#series=/data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells/*
	#for i in $series; do 
	#	dicom=$(echo ${i}|cut -d'/' -f10 |sed s@'/'@' '@g)
	...
	#cp $general/NODDI_B_700_Series0014/* /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells
	#series=/data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/shells/*
	#for i in $series; do 
	#	dicom=$(echo ${i}|cut -d'/' -f10 |sed s@'/'@' '@g)
	...
	inputnifti=${out}/unproc/shells/*.nii.gz	
	...
	unroundedbval=$(echo ${out}/unproc/shells/*.bval)
	bvec=$(echo ${out}/unproc/shells/*.bvec)
	inputnifti=$(echo ${inputnifti}) 
	echo $inputnifti
	...
	bet ${inputnifti} ${init_mask} -m -f 0.2
	...
	for ((i=1; i<=116; i+=1)); do indx="$indx 1"; done
	...
	# Grab Magnitude B0	
	#cp $general/B0map_v4_Series0009/* /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/B0_Magnitude
	...
	# Make negative image so it looks like white matter
	#fslmaths Mag_B0_Betted_Ero -mul -1 Mag_B0_Betted_Ero_Neg.nii.gz
	
	# Grab phase B0
	#cp $general/B0map_v4_Series0010/* /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/unproc/B0_Phase
	...
	# Take Fieldmap from T1 space to sequence (diffusion) space via translation aka affine
	...
	#fugue -i ${eddy_output} --dwell=0.0009 --loadfmap=$out/coreg/${IDs}_seqSpaceFieldMap.nii.gz -u ${IDs}_fugued.nii.gz
	...
	# Affine DWI to T1  (prep for undistorting)
