...
##c3d_affine_tool -src /data/jux/daviska/apines/atlases/MNI152_T1_1mm_brain.nii.gz -ref ${out}/coreg/${IDs}_T1_betted.nii.gz $out/coreg/${IDs}_MNI2StructFSL.mat -fsl2ras -oitk $out/coreg/${IDs}_MNIStructRas.mat
...
antsApplyTransforms -d 3 -e 0 -i /data/jux/daviska/apines/atlases/AAL3_1mm_flipx.nii.gz -r /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/coreg/${IDs}_DWISpaceT1.nii.gz -o $out/coreg/${IDs}_DWISpace_AALredo.nii.gz -t [$out/coreg/${IDs}_MultiShDiff2StructRas.mat,1] -t /data/jux/daviska/apines/3T_Subjects_NODDI/${IDs}/coreg/${IDs}_MNI_T1_Warp1Warp.nii.gz -t $out/coreg/${IDs}_MNI_T1_Warp0GenericAffine.mat -n NearestNeighbor
...
### Make a super sparse atlas for prob testing
###fslmaths ${subAAL} -uthr 4012 -thr 4011 ${out}/subAAL_sparse.nii.gz
...
dsource=$out/AMICO/dwibet.nii.gz
...
# Probtrack (PICO)
##${cdir}/dtlutgen -schemefile $out/tractography/${IDs}.scheme -snr 28 > ${out}/picotable.dat
...
##THIScat $out/tractography/${IDs}_WdtModelFit.Bdouble | ${cdir}/picopdfs -inputmodel dt -luts ${out}/picotable.dat | ${cdir}/track -inputmodel pico -seedfile ${subAAL} -iterations 2000 -curvethresh 80 -anisthresh 0.05 -anisfile ${out}/tractography/${IDs}_Camino_FA.nii.gz > ${out}/tractography/${IDs}_picotracts_FA_Thresh.Bfloat ##| ${cdir}/procstreamlines -seedfile ${subAAL} -outputcp -outputroot ${out}/pico/ -iterations 50 
...
##/data/jux/daviska/apines/camino-trackvis-0.2.8.1/bin/camino_to_trackvis -i ${out}/${IDs}_bayestracts.bfloat -o ${out}/tractography/${IDs}_streamlinesaapc.trk --nifti $out/prestats/${IDs}_eddied_undistort_warped_t1Masked.nii.gz --phys-coords

###/data/jux/daviska/apines/camino-trackvis-0.2.8.1/bin/camino_to_trackvis -i ${out}/pc_streams.Bdouble -o ${out}/tractography/${IDs}_streamlinesaapc.trk --nifti $out/prestats/${IDs}_eddied_undistort_warped_t1Masked.nii.gz --phys-coords
...
######$cdir/conmat -inputfile "${tractography_output}" -targetfile ${subAAL} -scalarfile $out/tractography/${IDs}_Camino_FA.nii.gz -tractstat mean -outputroot $out/tractography/${IDs}_FA_matrixaa
