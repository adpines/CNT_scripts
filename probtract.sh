...
$cdir/image2voxel -4dimage $out/AMICO/dwi.nii -outputfile $out/tractography/${IDs}_i2v.Bfloat
...
#Camino tractography
##$cdir/track -inputmodel dt -seedfile "${subAAL}" -inputfile "${model_fit_path}" -tracker euler -interpolator linear -iterations 10 -curvethresh 60 | $cdir/procstreamlines -exclusionfile "${exclusion_path}" -truncateinexclusion -outputfile "${tractography_output}"
